...
contract SimpleStorage is ERC721, ERC721URIStorage, Ownable {
    ...
    constructor(bool status, uint256 head, uint256 tail, address _royaltyToken) ERC721("Patent Hunter", "PATH") {
        _status = status;
    ...
    function safeMint(uint256 patentNumber, string memory memo) public {
        ...
        if (patentNumber < publicationNumberHead || patentNumber > publicationNumberTail) {
            revert SimpleStorage_InvalidPatentNumber();
        }

        if(keccak256(abi.encodePacked(memo)) == keccak256(abi.encodePacked(""))) {
        ...
        _setTokenURI(tokenId, string.concat(baseUrl, Strings.toString(patentNumber)));
    ...
    function getTokenFromPatent(uint256 patentNumber) public view returns(uint256) {
    ...
    function payRoyaltyFee(address from, address to, uint256 amount) public {
